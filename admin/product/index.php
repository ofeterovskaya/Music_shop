...
switch ($action) {
	...
	case 'add_category':
        ...
        $category_iddd = add_category($category_name);
		$categories = get_categories();
        include('category_list.php');
        break;
		...
		$category_size= category_size($category_id);
		if ($category_size==0){
			 $category_iddd = delete_category($category_id);
		}
		else{
			$error = 'This category cannot be deleted because it contains product';
